...
function get_location_nav(string $view, int $view_all, int $year, int $month, int $day, int $area, int $room) : string
{
  ...
  $html .= '<button id="viewBookingsBtn">View All Bookings</button>';

$html .= '<link rel="stylesheet" href="css/mrbs-modal.css.php"></link>';

$html .= '<div id="bookingModal" class="booking-modal">
      <!-- Modal Content -->
  ...
  $html .= '<script src="js/modal.js.php" defer></script>';
...
function loadBookingDetails() {
    ...
    xhr.onreadystatechange = function() {
        if (xhr.readyState == 4 && xhr.status == 200) {
            document.getElementById("bookingDetailsContent").innerHTML = xhr.responseText;
        }
    };
    xhr.send();
}
...
function searchBookings() {
    ...
    table = document.querySelector(".booking-details-content table");
